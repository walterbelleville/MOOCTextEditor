...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(int i = 0; i < tokens.size(); i++) {
			...
			boolean isAWord = isWord(word);
			//System.out.println("word: " + word);
			 
			if (!isAWord) { // find punctuation and count sentences
				getNumSentences();
				
			} else if ( isAWord ) { // count number of words
				
				if( isAWord && tokens.size() - 1 == i)  { // Check if end of token string is word. 
					getNumSentences();
					//System.out.println("end of token string: . " + "index: " + i);
